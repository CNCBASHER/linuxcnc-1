...
int emcec_el2xxx_init(int comp_id, struct emcec_slave *slave, ec_pdo_entry_reg_t *pdo_entry_regs) {
  ...
  for (i=0, pin=hal_data; i<slave->pdo_entry_count; i++, pin++) {
    ...
    // export pins
    if ((err = hal_pin_bit_newf(HAL_IN, &(pin->out), comp_id, "%s.%d.%d.dout-%d", EMCEC_MODULE_NAME, master->index, slave->index, i)) != 0) {
      rtapi_print_msg(RTAPI_MSG_ERR, EMCEC_MSG_PFX "exporting pin %s.%d.%d.dout-%02d failed\n", EMCEC_MODULE_NAME, master->index, slave->index, i);
      return err;
    }
    if ((err = hal_param_bit_newf(HAL_RW, &(pin->invert), comp_id, "%s.%d.%d.dout-%d-invert", EMCEC_MODULE_NAME, master->index, slave->index, i)) != 0) {
      rtapi_print_msg(RTAPI_MSG_ERR, EMCEC_MSG_PFX "exporting pin %s.%d.%d.dout-%02d-invert failed\n", EMCEC_MODULE_NAME, master->index, slave->index, i);
      return err;
    }
